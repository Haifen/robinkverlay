...
LANG_SUPPORT_EXTENDED=( clojure go java lua php pypy python python_asyncio python_gevent ruby )
...
src_install() {
	...
	use clojure && dosym uwsgi /usr/bin/uwsgi_ring
	use go && dosym uwsgi /usr/bin/uwsgi_go
	use java && dosym uwsgi /usr/bin/uwsgi_jwsgi
...
pkg_postinst() {
	...
	use clojure   && elog "  '--plugins ring for clojure/ring"
	use java      && elog "  '--plugins jwsgi for java/jwsgi"
