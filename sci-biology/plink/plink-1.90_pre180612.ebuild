...
MY_SV="${P##plink-1.90_pre}"
SRC_URI="https://www.cog-genomics.org/static/bin/plink${MY_SV}/plink_src.zip -> ${P}.zip"
...
src_compile() {
	local blasflags
	use lapack && blasflags="$($(tc-getPKG_CONFIG) --cflags --libs lapack cblas)"
	emake \
		CXX=$(tc-getCXX) \
		CFLAGS="${CFLAGS}" \
